"""Native cognis-connect emit for txgraph — forward findings to any platform.

Maps txgraph's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    txgraph ... --format json | txgraph-emit --to stix
    txgraph-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
