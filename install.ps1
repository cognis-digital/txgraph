# Comprehensive installer for cognis-digital/txgraph (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# txgraph is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "txgraph"
$Url  = "git+https://github.com/cognis-digital/txgraph.git"
$Git  = "https://github.com/cognis-digital/txgraph.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: txgraph"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: txgraph"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: txgraph"; exit 0 }
}
