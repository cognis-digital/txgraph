...
[![PyPI](https://img.shields.io/pypi/v/cognis-txgraph.svg)](https://pypi.org/project/cognis-txgraph/)
[![CI](https://github.com/cognis-digital/txgraph/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/txgraph/actions)
...
TXGRAPH exists for one job — builds a transaction graph from ledger/account data and surfaces structuring, layering, and mule-network patterns for aml triage. — and does it without a SaaS bill or heavyweight setup.
...
```bash
txgraph --version
txgraph scan .                      # scan the current project
txgraph scan . --format json
txgraph scan . --fail-on high       # non-zero exit for CI gates
txgraph mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **AMLSim / Neo4j fraud graph**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`txgraph` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
